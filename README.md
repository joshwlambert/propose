...
* [exploreringbp](https://github.com/epiforecasts/exploreringbp/tree/master) is a Shiny app that was developed alongside the initial development of the `{ringbp}` package. It allowed for running scenarios under different parameterisations and plotting aspects of simulated outbreaks using `{ringbp}`. exploreringbp is not actively maintained and is not currently compatible with the recent updates in `{ringbp}`.
